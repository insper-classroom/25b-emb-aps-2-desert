...
// GPIO Pins - CORRIGIDOS E CONSISTENTES
#define BTN_SHOOT_GPIO      14      // Botão de disparo (clique esquerdo)
#define BTN_E_GPIO          11      // Botão de trocar arma (tecla E) 
...
static gyro_bias_t calibrate_gyro_bias(void) {
    ...
    // LED apaga após calibração (controle inicia DESLIGADO)
...
static void setup_buttons(void) {
    ...
    gpio_init(BTN_E_GPIO);
    gpio_set_dir(BTN_E_GPIO, GPIO_IN);
    gpio_pull_up(BTN_E_GPIO);
    ...
    gpio_set_irq_enabled(BTN_E_GPIO, GPIO_IRQ_EDGE_FALL, true);
...
static void mpu6050_task(void *p) {
    ...
    printf("\n");
    printf("========================================\n");
    printf("=== CONTROLE DE MIRA GYRO + BOTOES ===\n");
    printf("========================================\n");
    ...
    printf("\n");
    printf("=== PROTOCOLO UART ===\n");
    printf("axis=0,1: Movimento mouse (gyro)\n");
    printf("axis=3:   Disparo (clique)\n");
    printf("axis=6:   Tecla E (trocar arma)\n");
    printf("axis=7:   Tecla R (reload)\n");
    printf("========================================\n\n");
    ...
    while (true) {
        ...
        if (xSemaphoreTake(toggle_sem, 0) == pdTRUE) {
            ...
            printf("\n****************************************\n");
            printf("*** CONTROLE %s ***\n", mouse_enabled ? "LIGADO" : "DESLIGADO");
            printf("*** LED %s ***\n", mouse_enabled ? "ACESO" : "APAGADO");
            printf("****************************************\n");
        }
